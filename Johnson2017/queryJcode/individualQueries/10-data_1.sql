...
select
  ...
  , uo.UrineOutput
from mp_hourly_cohort_1 mp
left join mp_vital_1 vi
  on  mp.icustay_id = vi.icustay_id
  and mp.hr = vi.hr
left join mp_gcs_1 gcs
  on  mp.icustay_id = gcs.icustay_id
  and mp.hr = gcs.hr
left join mp_uo_1 uo
  on  mp.icustay_id = uo.icustay_id
  and mp.hr = uo.hr
left join mp_bg_art_1 bg
  on  mp.hadm_id = bg.hadm_id
  and mp.hr = bg.hr
left join mp_lab_1 lab
