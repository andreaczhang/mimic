-- ############## CE_1 #################
-- REQUIRES MP_CODE_STATUS_1, MP_INTIME_OUTTIME_1, MP_BG_ART_1, MP_SERVICE_1, MP_OBS_COUNT_1
-- NOTE: I HAVE CHANGED DM INTO MP FOR CONSISTENCY
-- REQUIRES INTERMEDIATE SUBQUERY 



-- DROP TABLE IF EXISTS dm_cohort CASCADE;
CREATE TABLE final_cohort_1 AS
...
(
  ...
  select
    ...
    , max(case when CMO=1 then 1 else 0 end) as cmo  -- ###### removed cmo_notes = 1
    , max(DNR) as dnr
    , max(DNI) as dni
    , max(DNCPR) as dncpr
  from mp_code_status_1 
  group by icustay_id
)
...
(
  ...
  from MP_INTIME_OUTTIME_1 ce  
  inner join icustays ie
    on ce.icustay_id = ie.icustay_id
  left join mp_bg_art_1   -- ##############################
    on ie.hadm_id = mp_bg_art_1.hadm_id
    and ce.intime_hr <= mp_bg_art_1.charttime
    and ce.outtime_hr >= mp_bg_art_1.charttime
...
(
  -- min code status
    ...
    from mp_intime_outtime_1 ce
    inner join mp_code_status_1 cs   
...
left join mp_intime_outtime_1 ce   -- ###########
...
WHERE subject_id IN (SELECT subject_id FROM MP_COHORT_1)
